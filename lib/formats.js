  ...
  writer: function(output) {
    ...
    this.write = function(data, note, program) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        output.write('  <string name="' + entry.code + '"');
        if (entry.context) {
          output.write('          context="' + entities.encode(entry.context) + '"');
        }
        if (entry.note) {
          output.write('          note="' + entities.encode(entry.note) + '"');
        }
        if (entry.versionAddedIn) {
          output.write('          versionAddedIn="' + entities.encode(entry.versionAddedIn) + '"');
        }
        if (entry.versionRemovedIn) {
          output.write('          versionRemovedIn="' + entities.encode(entry.versionRemovedIn) + '"');
        }
        output.write('          section="' + entry.section + '"');
        output.write('          >"' + entry.text.replace('\'', '\\\'') + '"</string>');
      }
